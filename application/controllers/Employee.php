...
class Employee extends CI_Controller {
	...
	public function index()
	{
        ...
        if ($this->input->post()) {
            // Set form validation rules
             $this->form_validation->set_rules('dob', 'Date of Birth', 'trim|required|callback_check_dob');
             // Run form validation
             if ($this->form_validation->run() == FALSE) {
                // Validation failed, display validation errors
                $this->load->view('employee_view'); // Load your form view with validation errors
                } else {
                     // Validation passed, continue with your logic
                       // Get validated data
                        $dob = $this->input->post('dob');
                        // Process the validated data, e.g., save to database, perform further actions, etc.
                        $data = array(
                           'dob' => $dob,
                           // Other fields from the form...
                       );
                         // Assuming you have a model method to save data to the database
                         if ($this->employee_model->save_data($data)) {
                            // Data saved successfully
                            // You might want to redirect the user to a success page or show a success message
                            redirect('success_page');
                           } else {
                                // Failed to save data
                                // You might want to show an error message or handle the error appropriately
                                echo "Failed to save data.";
                               }
                    }
                } else {
                    // If the form is not submitted, load your form view
                     $this->load->view('employee_view');
                    }

	}
    // Custom validation callback function to check DOB
    public function check_dob($dob) {
        // Convert DOB to date object
        $dob_date = new DateTime($dob);
        // Calculate user's age
         $now = new DateTime();
          $age = $dob_date->diff($now)->y;
          // Check if age is between 18 and 35 years
          if ($age >= 18 && $age <= 35) {
             return TRUE; // Validation passed
         } else {
            $this->form_validation->set_message('check_dob', 'The {field} must be between 18 and 35 years old.');
	...
	$blood = $this->input->post('blood');		
        ...
        $this->form_validation->set_rules('fname', 'First Name', 'trim|required|min_length[3]|max_length[50]|alpha|xss_clean');
        // Set validation rules for last name
        $this->form_validation->set_rules('lname', 'Last Name', 'trim|required|min_length[3]|max_length[50]|alpha|xss_clean');
        $this->form_validation->set_rules('dob', 'Date of Birth', 'trim|required|callback_check_dob');
